...
\item Define the variable $small$ to be the node with the smaller value and define the variable $large$ to be the node with the larger value. Recurse the function with the left child of $small$ and the $large$. Assign the returned node to be the new left child of the node $small$. Notice that the R-balance property ensures that the left subtree has a d value 1 less than the d of the original tree.
...
\li $\func{small.left} \gets \func{new\_child.parent}$
